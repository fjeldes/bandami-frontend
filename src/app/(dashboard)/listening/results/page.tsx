...
export default function ListeningResultsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const data = await apiFetch<Evaluation>(`/evaluate/listening/${examId}/evaluation`);
  ...
  if (loading) return <div className="flex flex-col items-center justify-center py-16 gap-3"><span className="material-symbols-outlined text-[36px] text-primary animate-spin">progress_activity</span><p className="text-body-md text-on-surface-variant">Evaluating your listening...</p></div>;
  if (error) return <div className="text-center py-12"><p className="text-body-md text-error mb-4">{error}</p><button onClick={() => router.push("/listening")} className="text-primary font-semibold">Back</button></div>;
  ...
  return (
    <div>
      <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <h1 className="text-headline-md font-bold text-primary">Listening Results</h1>
        <div className="flex gap-3">
          ...
          <Link href="/listening" className="px-4 py-1.5 bg-primary-container text-on-primary-container rounded-lg text-label-sm font-semibold hover:opacity-90 transition-colors">Try Another</Link>
        </div>
      </div>
